...
{  							  
	...
	{   
		...
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)draw_point_one(x,y,POINT_COLOR);
			else if(mode==0)draw_point_one(x,y,BACK_COLOR);
			temp<<=1;
			y++;
...
//***********************自定义函数********************************//
void draw_point_four(u16 a,u16 b,u16 color){
	
	draw_point_one(a, b, color);
	draw_point_one(a+1, b, color);
	draw_point_one(a, b+1, color);
	draw_point_one(a+1, b+1, color);
}
...
void draw_all(int x,int y,const u8 *p){
	u16 a;
	u16 b;
	u16 c;
	u16 d;
	...
	switch(y)
	{
		case 0: {add_y = 0;break;}
		case 1: {add_y = 85;break;}
		case 2: {add_y = 170;break;}
	}
	c= 10 + add_x;
	d = 20 + add_y;
	a = 10; 
	...
	for(b=d;b<=d+49;b++){
		for(a=c;a<=c+49;a++){
			//draw_point_one(a, b, RGB888ToRGB565(*p ,1));
			shift_buf = (*p) + (*(p+1)<<8);
			draw_point_one(a, b, shift_buf);
			p += 2;
		}
...
//*-********************************ico*******************//
void ico(void){          //图标
	draw_all(0,0,computer_bmp);
	draw_all(0,1,mobilephone_bmp);
	draw_all(0,2,computer_bmp);
	
	draw_all(1,0,mobilephone_bmp);
	draw_all(1,1,computer_bmp);
	draw_all(1,2,mobilephone_bmp);
	
	draw_all(2,0,computer_bmp);
	draw_all(2,1,mobilephone_bmp);
	draw_all(2,2,computer_bmp);
}

void ico_2(void){          //图标
	draw_all(0,0,mobilephone_bmp);
	draw_all(0,1,computer_bmp);
	draw_all(0,2,mobilephone_bmp);
	
	draw_all(1,0,computer_bmp);
	draw_all(1,1,mobilephone_bmp);
	draw_all(1,2,computer_bmp);
	
	draw_all(2,0,mobilephone_bmp);
	draw_all(2,1,computer_bmp);
	draw_all(2,2,mobilephone_bmp);
}

void ico_name(int i,int j,u8 p[]){  //图标名
	int a;
	int b;
	a = 10+(i*80);
	b = 70+(j*85);
	LCD_ShowString(a,b,200,16,16,p);
	
}



void ico_name_all(void){
...
///////////////////////////////////////////////////////////////////////////////////////
void draw_line(int y){        //分割线
	int i=0;
	for(i=0;i<=330;i++){
		draw_point_one(i,y,BLACK);
		
	}
	
}

void draw_line_color(int y, u16 color){        //分割线
	int i=0;
	for(i=0;i<=330;i++){
		draw_point_one(i,y,color);
		
	}
	
}

void draw_ver_color(int x, u16 color){
	int i=0;
	for(i=0;i<=264;i++){
		draw_point_one(x,i,color);
		
	}
	
}
	
void draw_net(u16 color){
	draw_line_color(100,color);
	draw_line_color(180,color);
	
	draw_ver_color(80, color);
	draw_ver_color(160, color);
	draw_line(265);
	
}

void input_background(u16 color){
	int i=0,j=0;
	for(j=0;j<=330;j++){
		for(i=0;i<=264;i++){
			draw_point_one(j,i,color);
		}
	}
...
void time_display(void){        //显示时间
	...
	BACK_COLOR = 0x5656;
			...
			LCD_ShowNum(40+50,280,calendar.w_year,4,12);	
			
			LCD_ShowString(120+3-6,280,200,1,12,"/");
	
			if(calendar.w_month<10){
			LCD_ShowString(120+3,280,200,1,12,"0");
			LCD_ShowNum(70+50+6+3,280,calendar.w_month,1,12);	
		}
		else{
			LCD_ShowNum(70+50+3,280,calendar.w_month,2,12);
		}
		LCD_ShowString(70+50+6+3+5,280,200,1,12,"/");
		
		
		if(calendar.w_date<10){
			LCD_ShowString(134+6,280,200,1,12,"0");
			LCD_ShowNum(85+50+6+6,280,calendar.w_date,1,12);
		}
		else{
			LCD_ShowNum(85+50+6,280,calendar.w_date,2,12);
		}
		...
		/////////////////////////////////////
		if(calendar.hour<10){
			LCD_ShowString(90+3,295,200,1,12,"0");
			LCD_ShowNum(40+50+6+3,295,calendar.hour,1,12);
		}
		else{
			LCD_ShowNum(40+50+4,295,calendar.hour,2,12);
		}
		LCD_ShowString(40+50+6+5+5,295,200,1,12,":");
		
		if(calendar.min<10){
			LCD_ShowString(110+5,295,200,1,12,"0");
			LCD_ShowNum(60+50+6+5,295,calendar.min,1,12);
		}
		else{
			LCD_ShowNum(60+50+5,295,calendar.min,2,12);
		}
		LCD_ShowString(60+50+6+5+5,295,200,1,12,":");
		
		if(calendar.sec<10){
			LCD_ShowString(130+5,295,200,1,12,"0");
			LCD_ShowNum(80+50+6+5,295,calendar.sec,1,12);
		}
		else{
			LCD_ShowNum(80+50+5,295,calendar.sec,2,12);
		}
...
void set_time_display(void){
	...
	int aaa;
	aaa = 1;
	for(i=0;i<=13;i++){    //<<DEL and <<BACK
		if(data_input_set[i] != 0)
			aaa = 0;
	}
	if(aaa == 1){
		...
		LCD_ShowString(20,285,200,6,16,"<<DEL ");
			...
			LCD_ShowNum(40+50,280,data_input_set[13],1,12);	
			LCD_ShowNum(40+50+6,280,data_input_set[12],1,12);	
			LCD_ShowNum(40+50+6+6,280,data_input_set[11],1,12);	
			LCD_ShowNum(40+50+6+6+6,280,data_input_set[10],1,12);	
			LCD_ShowString(120+3-6,280,200,1,12,"/");
	
			
			
			LCD_ShowNum(120+3,280,data_input_set[9],1,12);	
			LCD_ShowNum(70+50+6+3,280,data_input_set[8],1,12);	
			...
			LCD_ShowNum(134+6,280,data_input_set[7],1,12);
			LCD_ShowNum(85+50+6+6,280,data_input_set[6],1,12);
		...
		/////////////////////////////////////
			...
			LCD_ShowNum(90+5,295,data_input_set[5],1,12);
			LCD_ShowNum(40+50+6+5,295,data_input_set[4],1,12);
		
		
			LCD_ShowString(40+50+6+5+5,295,200,1,12,":");
		

			LCD_ShowNum(110+5,295,data_input_set[3],1,12);
			LCD_ShowNum(60+50+6+5,295,data_input_set[2],1,12);
		
		
			LCD_ShowString(60+50+6+5+5,295,200,1,12,":");
		
		
			LCD_ShowNum(130+5,295,data_input_set[1],1,12);
			LCD_ShowNum(80+50+6+5,295,data_input_set[0],1,12);
